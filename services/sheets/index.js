...
export const consultarTurnosPorFecha = async (fecha) => {
  try {
    let turnos = {
      Fecha: [],
      Inicio: [],
...
/* 
export const cambiarTurno = async (telefono, nuevaFecha, nuevaHoraInicio) => {
  try {
    const CREDENTIALS = {
      type: "service_account",
      project_id: "calendar-turnos-400220",
    ...
    // Buscar el turno por el número de teléfono.
    let turno = rows.find(row => row.Telefono === telefono);

    // Verificar que el turno exista y que la fecha sea futura.
    if (!turno || new Date(turno.Fecha) <= new Date()) {
      console.error('No se encontró un turno futuro para este número de teléfono.');
      return;
    }

    // Actualizar la fecha y la hora de inicio del turno.
    turno.Fecha = nuevaFecha;
    turno.Inicio = nuevaHoraInicio;
    ...
    console.error('Ocurrió un error al actualizar el turno:', error);
    throw error;
  }
};

export const cancelarTurno = async (telefono) => {
  try {
    const CREDENTIALS = {
      type: "service_account",
      project_id: "calendar-turnos-400220",
