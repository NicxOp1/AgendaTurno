...
// Función para formatear la fecha en "dd/mm/yyyy"
  ...
  const consultarTurnos= async(telefono)=> {
    let turnos = {
      'Fecha': [],
      'Hora de inicio': [],
    ...
    for (let index = 0; index < rows.length; index++) {
      ...
      if (row._rawData[5] === telefono) { // Suponiendo que el teléfono está en la sexta columna (índice 5).
    ...
    const agendarTurno = async(fecha, horaSolicitada, servicio, telefono) {
    // 1. Verificar si ya existe un turno con el mismo número de teléfono.
    const turnosExistentes = await consultarTurnosPorTelefono(telefono);
    ...
    await agregarTurno(fecha, horaSolicitada, servicio, telefono);
  
    return "Turno agendado con éxito.";
  }
  
  const consultarTurnosPorDiaYServicio = async (fecha, servicio) => {
    let turnos = {
      'Fecha': [],
      'Hora de inicio': [],
      'Hora de finalización': [],
