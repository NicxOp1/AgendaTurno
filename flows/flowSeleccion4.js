...
import {consultarTurnos, consultarTurnosPorDiaYServicio} from '../services/sheets/index.js'
...
.addAnswer(
   `📆 Porfavor elige la fecha que quieres consultar un turno. Recordá el formato DD/MM/AA 
   Ej: 12/02/23 `,
  ...
  async (ctx, { state, flowDynamic,gotoFlow,endFlow }) => {
    ...
    if(mensaje.contadorTurnos>=3){
        ...
        if(validarFecha(ctx.body)){
            let consulta = await consultarTurnosPorFecha(ctx.body)
